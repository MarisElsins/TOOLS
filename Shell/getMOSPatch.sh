...
set +e
wget --save-cookies=$COOK --keep-session-cookies --no-check-certificate --http-user $mosUser --ask-password "https://updates.oracle.com/Orion/SimpleSearch/switch_to_saved_searches" -O $TMP1 -o $TMP2 --no-verbose
if [ $? -ne 0 ] ; then
  cat $TMP2
  exit 1
fi
