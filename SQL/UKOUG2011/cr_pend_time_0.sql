...
with interv0 as (select to_date('&from_DDMMYYYY_HH24MISS','DDMMYYYY-HH24MISS') int_start, to_date('&to_DDMMYYYY_HH24MISS','DDMMYYYY-HH24MISS') int_end from dual),
    ...
    select  i.int_start, i.int_end,
            sum(case when greatest(request_date, requested_start_date) between i.int_start and  i.int_end then 1 else 0 end) req_scheduled,
            count(case when greatest(-1,least(actual_start_date,i.int_end) - greatest(greatest(request_date, requested_start_date),i.int_start))>0 then 1 else null end) req_pending,
            sum(case when actual_start_date between i.int_start and  i.int_end then 1 else 0 end) req_started,
            sum(case when actual_completion_date between i.int_start and  i.int_end then 1 else 0 end) req_completed,
    ...
    where phase_code='C'
        and greatest(request_date, requested_start_date) <= i0.int_end
        and actual_completion_date >= i0.int_start
