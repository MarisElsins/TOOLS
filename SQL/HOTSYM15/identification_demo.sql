...
select sql_id, exact_matching_signature, force_matching_signature, sql_text, length(sql_text) sql_length from v$sql where sql_text like '%/*test'||'query%*/%' order by 3, LAST_LOAD_TIME;
pause

-- This doesn't really identofy the SQL statement, but take a look what happens if we group them by PLAN_HASH_VALUE
pause

clear breaks
break on PLAN_HASH_VALUE duplicates skip 1;
select sql_id, exact_matching_signature, force_matching_signature, plan_hash_value, sql_text, length(sql_text) sql_length from v$sql where sql_text like '%/*test'||'query%*/%' order by 4, LAST_LOAD_TIME;
pause
